...
import EmployeeLayout from '../ManagerLayout'; // Import the EmployeeLayout component
...
const ManagerClients = () => {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) throw new Error('Please log in to access team clients');
        if (response.status === 403) throw new Error('Access denied: Manager role required');
        throw new Error(`Failed to fetch team clients: ${response.status} ${response.statusText}`);
      }
      const data: Client[] = await response.json();
      setClients(data);
    } catch (err: any) {
      setError(err.message || 'Failed to load team clients');
      console.error('Manager Clients error:', err);
      toast.error(err.message || 'Failed to load team clients');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Wrap the content in EmployeeLayout
  return (
    <EmployeeLayout>
      <>
        <Toaster />
        {isLoading ? (
          <div className="max-w-7xl mx-auto p-6 flex items-center justify-center text-gray-700 min-h-screen">
            Loading team clients...
          </div>
        ) : error ? (
          <div className="max-w-7xl mx-auto p-6 flex items-center justify-center text-red-600 min-h-screen">
          ...
          <div className="max-w-7xl mx-auto p-6">
            <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Team Clients</h2>
              {clients.length === 0 ? (
                <p className="text-gray-600">No team clients found.</p>
              ) : (
